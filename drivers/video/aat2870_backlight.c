...
static int aat2870_backlight_set_brightness(struct udevice *dev, int percent)
{
	...
	ret = dm_i2c_reg_write(dev, AAT2870_BLM,
			       brightness);
	if (ret)
		return ret;
...
static int aat2870_backlight_probe(struct udevice *dev)
{
	...
	ret = uclass_get_device_by_phandle(UCLASS_REGULATOR, dev,
					   "power-supply", &power);
	if (ret) {
		log_err("%s: cannot get power-supply (%d)\n",
			__func__, ret);
		return ret;
	}

	ret = regulator_set_enable_if_allowed(power, 1);
	if (ret) {
		log_err("%s: enabling power-supply failed (%d)\n",
			  __func__, ret);
		return ret;
	}

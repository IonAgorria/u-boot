 ...
 * Bit Stream Engine for Video (BSEV) hardware block and
 * Bit Stream Engine for Audio (BSEA).
 *
...
const u32 TEGRA_AES_IRAM_MAX_ADDR = IRAM_BASE | TEGRA_AES_KEYTABLEADDR_FIELD;
...
bool tegra_aes_is_busy(struct tegra_priv *priv, bool dma_wait) {
...
u32 tegra_aes_check_error(struct tegra_priv *priv) {
...
int tegra_aes_wait_for_idle_dma(struct tegra_priv *priv, bool dma_wait) {
    ulong start = get_timer(0);
    while (tegra_aes_is_busy(priv, dma_wait)) {
        if (TEGRA_AES_BUSY_TIMEOUT_MS < get_timer(start)) {
            log_err("%s: TIMEOUT!!!\n", __func__);
            break;
        }
        mdelay(5);
...
int tegra_aes_wait_for_idle(struct tegra_priv *priv) {
    return tegra_aes_wait_for_idle_dma(priv, 1);
}

int tegra_aes_configure(struct tegra_priv *priv) {
    u32 value;
...
int tegra_aes_select_key_slot(struct tegra_priv *priv, u32 key_size, u8 slot) {
    if (tegra_aes_wait_for_idle(priv))
        return -1;
...
int tegra_aes_call_engine(struct tegra_priv *priv, u8 *src, u8 *dst,
...
int tegra_aes_process_blocks(struct udevice *dev, u8 *iv, 
                             u8 *src, u8 *dst, u32 num_aes_blocks,
                             u32 mode)
{
    struct tegra_priv *priv = dev_get_priv(dev);
...
void tegra_aes_test_loaded_sbk(struct udevice *dev)
{
    ...
    if (memcmp(priv->iram_addr, AES_ZERO_BLOCK, AES_BLOCK_LENGTH) == 0) {
        log_err("%s: engine is not operational! (opmode 0x%x)\n", __func__, opmode);
    } else if (memcmp(priv->iram_addr, ZERO_KEY_CIPHERTEXT, AES_BLOCK_LENGTH) == 0) {
...
int tegra_aes_bind(struct udevice *dev)
{
    log_debug("%s\n", __func__);
...
int tegra_aes_probe(struct udevice *dev)
{
    int ret;
    u32 value;
    bool is_bsev;
    ...
    if ((value & 0xFFF0000F) != IRAM_BASE || value > TEGRA_AES_IRAM_MAX_ADDR) {
        log_err("%s: iram buffer must be located inside iram, AES block aligned and not "
                  "above 0x%08x, current addr %p\n", __func__, TEGRA_AES_IRAM_MAX_ADDR, priv->iram_addr);
        return -EINVAL;
    }

    //Get engine reset and clk
    ret = reset_get_by_name(dev, "engine", &priv->reset_ctl);
    if (ret) {
        log_err("%s: reset_get_by_name() engine failed: %d\n", __func__, ret);
        return ret;
    }

    ret = clk_get_by_index(dev, 0, &priv->clk);
    if (ret) {
        log_err("%s: clk_get_by_index() clk failed: %d\n", __func__, ret);
        return ret;
    }

    //If is BSEV we need VDE reset and clk
    if (is_bsev) {
        ret = reset_get_by_name(dev, "vde", &priv->reset_ctl_vde);
        if (ret) {
            log_err("%s: reset_get_by_name() vde failed: %d\n", __func__, ret);
            return ret;
        }

        ret = clk_get_by_index(dev, 1, &priv->pclk);
        if (ret) {
            log_err("%s: clk_get_by_index() pclk failed: %d\n", __func__, ret);
            return ret;
        }
    }

    //Assert resets on VDE if BSEV and engine
    if (is_bsev) {
        ret = reset_assert(&priv->reset_ctl_vde);
        if (ret) {
            log_err("%s: reset_assert() vde failed: %d\n", __func__, ret);
            return ret;
        }
    }
    ret = reset_assert(&priv->reset_ctl);
    if (ret) {
        log_err("%s: reset_assert() failed: %d\n", __func__, ret);
        return ret;
    }

    //Enable VDE clocks if BSEV
    if (is_bsev) {
        ret = clk_enable(&priv->pclk);
        if (ret) {
            log_err("%s: clk_enable() pclk failed: %d\n", __func__, ret);
            return ret;
        }

        ret = clk_set_rate(&priv->pclk, 50000000);
        if (IS_ERR_VALUE(ret)) {
            log_err("%s: clk_set_rate() pclk failed: %d\n", __func__, ret);
            return ret;
        }
    }
    ...
    if (ret) {
        log_err("%s: clk_enable() clk failed: %d\n", __func__, ret);
        return ret;
    }

    //Deassert resets on VDE and BSEV
    if (is_bsev) {
        ret = reset_deassert(&priv->reset_ctl_vde);
        if (ret) {
            log_err("%s: reset_deassert() vde failed: %d\n", __func__, ret);
            return ret;
        }
    }
    ret = reset_deassert(&priv->reset_ctl);
    if (ret) {
        log_err("%s: reset_deassert() failed: %d\n", __func__, ret);
        return ret;
    }
...
int tegra_aes_ops_available_key_slots(void) {
    return 8;
}

int tegra_aes_ops_select_key_slot(struct udevice *dev, u32 key_size, u8 slot) {
    struct tegra_priv *priv = dev_get_priv(dev);
...
int tegra_aes_ops_set_key_for_key_slot(struct udevice *dev, u32 key_size, u8 *key, 
                                 u8 slot) {
    struct tegra_priv *priv = dev_get_priv(dev);
...
int tegra_aes_ops_aes_ecb_encrypt(struct udevice *dev, u8 *src,
                                  u8 *dst, u32 num_aes_blocks) {
    return tegra_aes_process_blocks(dev, NULL, src, dst, num_aes_blocks,
                                    TEGRA_AES_MODE_ENCRYPT);
}

int tegra_aes_ops_aes_ecb_decrypt(struct udevice *dev, u8 *src,
                                  u8 *dst, u32 num_aes_blocks) {
    return tegra_aes_process_blocks(dev, NULL, src, dst, num_aes_blocks,
                                    0);
}

int tegra_aes_ops_aes_cbc_encrypt(struct udevice *dev, u8 *iv,
                                  u8 *src, u8 *dst, u32 num_aes_blocks) {
    return tegra_aes_process_blocks(dev, iv, src, dst, num_aes_blocks,
                                    TEGRA_AES_MODE_CBC | TEGRA_AES_MODE_ENCRYPT);
}

int tegra_aes_ops_aes_cbc_decrypt(struct udevice *dev, u8 *iv,
                                  u8 *src, u8 *dst, u32 num_aes_blocks) {
    return tegra_aes_process_blocks(dev, iv, src, dst, num_aes_blocks,
                                    TEGRA_AES_MODE_CBC);
}

struct aes_ops tegra_aes_ops = {
...
const struct udevice_id tegra_aes_ids[] = {
...
U_BOOT_DRIVER(tegra_aes) = {
	...
	.priv_auto = sizeof(struct tegra_priv),
	.bind = tegra_aes_bind,
